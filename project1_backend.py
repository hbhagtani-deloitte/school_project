...
def studentData():
        con = mysql.connector.connect(host="localhost",user="root",passwd="Mynameis007",database="project")
        cur=con.cursor()
...
def addStdRec(StdID,Firstname,Surname,DoB,Age,Gender,Address,Mobile):
        con=con = mysql.connector.connect(host="localhost",user="root",passwd="Mynameis007")
        cur=con.cursor()
...
def dataUpdate(id,StdID="",Firstname="",Surname="",DoB="",Age="",Gender="",Address="",Mobile=""):
        con = mysql.connector.connect(host="localhost",user="root",passwd="Mynameis007")
        cur=con.cursor()
        cur.execute("use project")
